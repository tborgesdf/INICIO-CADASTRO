...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    try {
      const [rows]: any = await conn.query(`SELECT id, name, cpf, email, phone, latitude, longitude, visa_type, status, created_at FROM users WHERE account_id=? ORDER BY created_at DESC LIMIT 1`, [sess.accountId]);
      const user = (rows||[])[0] || null;
      if (!user) return res.status(200).json({ ok: true, latest: null });
      const [social]: any = await conn.query(`SELECT platform, handle FROM user_social_media WHERE user_id=? ORDER BY id ASC`, [user.id]);
      const [countries]: any = await conn.query(`SELECT country FROM user_countries WHERE user_id=? ORDER BY id ASC`, [user.id]);
      return res.status(200).json({ ok: true, latest: user, social, countries });
